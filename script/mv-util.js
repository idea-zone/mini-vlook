import { appendBlock, deleteBlock, exportMdContent, getBlockAttrs, insertBlock, prependBlock, setBlockAttrs, updateBlock } from "./api.js";
 ...
 * @returns 
 */
mv.UpdateBlockByDom_API = async(id,dom)=>{
    if (mv.Empty(id)) return null;
    let rt =await updateBlock(
        id,
        'dom',
    ...
    return await rt[0].doOperations[0].id;
}
...
/**
 * 用 md 在 pid 对应块中前面插入孩子节点
 * @param {*} pid 
 * @param {*} md 
 * @returns 
 */
mv.InsertPrependBlockByMd_API = async(pid,md)=>{
    if (mv.Empty(pid)) return null;
    let rt =await prependBlock(
        pid,
        'markdown',
        md
    );
